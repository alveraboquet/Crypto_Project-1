...
async def buysocket(ticker, buy_amt, SL=0.99, Target=1.01, open_position=False):
    ...
    while open_position:    
        ...
        df = createframe(msg)
        print(f'close = {df.Price.values};')
        i = i+1
        if i == 10:
            break
        #target = '{buyprice * Target}; stop = {buyprice * SL}')
...
if __name__ == "__main__":
    ...
    threads = [None]*3
    #threads[1] = threading.Thread(target=asyncio.run, args=(buysocket('1INCHUSDT', 0.0005),))
    #threads[1].start()
    threading.Thread(target=asyncio.run, args=(buysocket('1INCHUSDT', 0.0005),)).start()

    threading.Thread(target=asyncio.run, args=(buysocket('BTCUSDT', 0.0005),)).start()
    #threads[0] = threading.Thread(target=asyncio.run, args=(buysocket('BTCUSDT', 0.0005),))
    #threads[0].start()
