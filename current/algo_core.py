...
if __name__ == "__main__":
    ...
    # Load binance client and database config
    client = binance_config()
    ...
    while(True):
        # Wait for trend line data to be updated
        if(waiting):
        ...
        if(check_data):
            ...
            # Update var for current data
            date_recent_checked = new_trends.date_request.max()

            # Pivot data with 
            try:
                trends = new_trends.pivot(index="date", columns="crypto_id", values="hype") #https://pandas.pydata.org/pandas-docs/stable/user_guide/reshaping.html
            except:
