...
coins_id_list = coins_id_list[199:]
tradedata = []
for coin in coins_id_list:
    #time = str((60*24*7*10)+3)
    time_var = str((60*24*((7*12)-2)))
    try:
    ...
    for x in [0, 5, 10, 15, 20]:
        ...
        tradedata.append([coin, result[1]])
    # backtest(HYPE, CLOSE)
    # pd.set_option('display.max_rows', None, 'display.max_columns', None)
    # print(HYPE)
    # with open("tradedata_output.txt", "w") as txt_file:
    #     for line in tradedata:
    #         txt_file.write(line + "\n")
    with open("tradedata_output.csv", "w") as spreadsheet:
        wr = csv.writer(spreadsheet)
        wr.writerows(tradedata)
...
#check coin names for accurate results : mina -> mina coin


#end up with dataframe of 'coin close' and 'indicator' with a dictionary of the thresholds ( and before done what type of trend)
#or just indicators
#compare new line row of indicator (if any indicator > threshold): buy that ind